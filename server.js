...
(async () => {
  ...
  //Config page
  app.get('/servers/:id/config', async (req, res) => {
    if (!req.params.id || req.params.id === 'undefined')
      return res.send({ status: 'ERROR' });

    const result = await db.get('SELECT * FROM servers WHERE id = ?', [req.params.id]);
    res.render('config.ejs', { server: result });
  });
  ...
  app.post('/api/server/:id/change/property', async (req, res) => {
    const serverID = req.params.id;
    ...
    try {
      setServerProperty(`minecraft/servers/${serverID}`, key, value);
    } catch (e) {
      return res.send({ status: 'ERROR' });
    }
  ...
  app.get('/api/servers/:id/config/properties', async (req, res) => {
    try {
      res.send({ status: 'OK', properties: getServerProperties(`minecraft/servers/${req.params.id}`) });
    } catch (e) {
...
function getProjectFromHash(modHash, file) {
  return new Promise((resolve) => {
    ...
    fetch(versionUrl).then((response) => {
      response.json().then((version) => {
        fetch(`https://api.modrinth.com/v2/project/${version.project_id}`).then((r) => {
          r.json().then((project) => {
            project.file = file;
            project.body = md.render(project.body);
...
async function getModList(serverDir) {
  ...
  for (let i = 0; i < mods.length; i++) {
    ...
    modList.push(getProjectFromHash(modHash, mod));
  }

  modList = await Promise.allSettled(modList);
  return modList;
}
...
function getServerProperties(directory) {
  return properties.parse(fs.readFileSync(`${directory}/server.properties`, 'utf8'));
}

function setServerProperty(directory, key, value) {
  const editor = properties.createEditor(`${directory}/server.properties`);
  editor.set(key, value);
  editor.save(`${directory}/server.properties`);
}